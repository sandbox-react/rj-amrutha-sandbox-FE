...
const Pages = () => {
    ...
    const fetchPages = async (withLoader = false) => {
        try {
            setApiError("");
            ...
            const response = await getData("/cms/pages");
            setRows(response?.data || []);
        } catch (error) {
            setApiError(error?.message || "Failed to fetch pages");
        } finally {
            setLoading(false);
    ...
    const handleDelete = async (row) => {
        const confirmed = window.confirm(`Delete page "${row.title}"?`);
        if (!confirmed) return;
        ...
        try {
            ...
            await deleteData(`/cms/pages/${row.id}`);
            setSuccessMsg("Page deleted successfully");
            fetchPages(true);
        } catch (error) {
            setApiError(error?.message || "Failed to delete page");
        }
    };
                ...
                >
                    <Box>
                        ...
                        <Typography sx={{ mt: 0.5, color: "text.secondary" }}>
                            Manage content pages and meta tags.
                        </Typography>
                    </Box>
                </Box>
                    ...
                    <Table>
                        <TableHead>
                            <TableRow>
                                ...
                                <TableCell sx={headCellSx}>Meta Title</TableCell>
                            ...
                            {tableLoading ? (
                                ...
                                rows.map((row) => (
                                    <TableRow key={row.id} hover>
                                        ...
                                        <TableCell sx={bodyCellSx}>
                                            <Typography color="text.secondary">{row.slug}</Typography>
                                        </TableCell>
